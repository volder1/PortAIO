...
namespace LCS_LeBlanc.Modes
{
    internal static class Clear
    {
        public static void JungleInit()
        {
            ...
            if (ObjectManager.Player.ManaPercent < Utilities.Slider("jungle.mana", Menus.jungleClear) && (mob == null || mob.Count == 0))
            {
                return;
            }

            if (Spells.Q.IsReady() && Utilities.Enabled("q.jungle", Menus.jungleClear) && mob[0].IsValidTarget(Spells.Q.Range))
            {
                Spells.Q.CastOnUnit(mob[0]);
            }

            if (Spells.W.IsReady() && Utilities.Enabled("w.jungle", Menus.jungleClear) && mob[0].IsValidTarget(Spells.W.Range))
            {
                Spells.W.Cast(mob[0].Position);
            }

            if (Spells.E.IsReady() && Utilities.Enabled("e.jungle", Menus.jungleClear) && mob[0].IsValidTarget(Spells.E.Range))
            {
        ...
        public static void WaveInit()
        {
            ...
            if (ObjectManager.Player.ManaPercent < Utilities.Slider("clear.mana", Menus.laneclearMenu) && (minion == null || minion.Count == 0))
            {
                return;
            }

            if (Spells.Q.IsReady() && Utilities.Enabled("q.lasthit", Menus.laneclearMenu))
            {
            ...
            if (Spells.W.IsReady() && Utilities.Enabled("w.clear", Menus.laneclearMenu))
            {
                var aeominpos = Spells.W.GetCircularFarmLocation(minion);
                if (aeominpos.MinionsHit >= Utilities.Slider("w.hit.x.minion", Menus.laneclearMenu))
                {
