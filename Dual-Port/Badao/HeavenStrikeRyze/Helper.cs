...
namespace HeavenStrikeRyze
{
    public static class Helper
    {
        ...
        public static double Qdamage(Obj_AI_Base target, bool hasbuffe = false)
        {
            ...
            return Player.CalcDamage(target, DamageType.Magical, damage * (1 + bonus));
        }
        public static double Wdamge(Obj_AI_Base target)
        {
            ...
            var damage = new double[] { 80, 100, 120, 140, 160 }[Program._w.Level - 1]
                ...
                + 0.1 * BonusMana;
            return Player.CalcDamage(target, DamageType.Magical, damage);
        }
        ...
        public static List<Obj_AI_Base> GetchainedTarget(Obj_AI_Base target)
        {
            ...
            if (!HasEBuff(target))
                return targets;
            targets.Add(target);
            var hasbuff = ObjectManager.Get<Obj_AI_Base>().Where(x => !x.IsAlly && HasEBuff(x) && !x.IsDead && x.LSDistance(target) <= 1000);
            int count = 1;
            while (count == 1)
            {
                count = 2;
                ...
                {
                    if (HasEBuff(tar) && !tar.IsDead && targets.Any(x => x.LSDistance(tar.Position) <= 300) && !targets.Any(x => x.NetworkId == tar.NetworkId))
                    {
                        targets.Add(tar);
                        count = 1;
                    }
