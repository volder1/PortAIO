    ...
    public static class ConePrediction
    {
            ...
            Vector2 from, Vector2 rangeCheckFrom)
        {
            return Prediction.GetPrediction(target, width, delay, missileSpeed, range, collisionable,
                SkillshotType.SkillshotCone, path, avgt, movt, avgp, anglediff, from, rangeCheckFrom);
        }
                ...
                if (prediction.HitChance > EloBuddy.SDK.Enumerations.HitChance.Medium)
                {
                    ...
                    var collidedEnemies =
                        HeroManager.Enemies.AsParallel()
                            .Where(
                                p =>
                                    !spellHitBox.IsOutside(Prediction.GetFastUnitPosition(p, delay, missileSpeed, from)));
