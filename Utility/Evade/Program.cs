 ...
 namespace EvadeSharp
{
    internal class Program
    {
        ...
        public static void Game_OnGameStart()
        {
        ...
        static void Spellbook_OnCastSpell(Spellbook sender, SpellbookCastSpellEventArgs args)
        {
            if (sender.Owner.IsValid && sender.Owner.IsMe)
            {
                ...
                if (Evading)
                {
                    var blockLevel = getBoxItem(Config.misc, "BlockSpells");
                    ...
                    foreach (var skillshot in DetectedSkillshots)
                    {
                        ...
                        {
                            isDangerous = getCheckBoxItem(Config.skillShots, "IsDangerous" + skillshot.SpellData.MenuItemName);
                            if (isDangerous)
                            {
                                break;
                            }
        ...
        private static void UnitOnOnDash(Obj_AI_Base sender, Dash.DashItem args)
        {
            if (sender.IsMe)
            {
                if (Config.PrintSpellData)
                {
                    //Console.WriteLine(Utils.TickCount + "DASH: Speed: " + args.Speed + " Width:" + args.EndPos.LSDistance(args.StartPos));
                }
                        ...
                        if (evadeSpell.IsDash)
                        {
                            ...
                            if (evadeSpell.IsTargetted) //Lesinga W.
                            {
                                ...
                                if (evadeSpell.IsTargetted &&
                                    ...
                                    getCheckBoxItem(Config.evadeSpells, "WardJump" + evadeSpell.Name))
                                {
        ...
        {
            if (!getCheckBoxItem(Config.drawings, "EnableDrawings"))
            {
                return;
            }

            if (!getCheckBoxItem(Config.misc, "ShowEvadeStatus"))
            {
            ...
            var Border = getSliderItem(Config.drawings, "Border");
            var missileColor = Color.LimeGreen;
            
            //Draw the polygon for each skillshot.
            foreach (var skillshot in DetectedSkillshots)
            {
                skillshot.Draw(
                    (skillshot.Evade() && getKeyBindItem(Config.Menu, "Enabled"))
                        ? Color.White
                        : Color.Red, missileColor, Border);
            }
